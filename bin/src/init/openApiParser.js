...
async function openAPIParser(api) {
  ...
  // fs.writeFileSync(path.resolve(__dirname, 'openAPI-res.json'), JSON.stringify(deref));
  const batConfig = {
    serverLocation: null,
    serverURL: deref.servers ? deref.servers[0].url : null,
...
// openAPIParser(path.resolve(__dirname, './openAPI-example-expanded.yml'))
//   .then((config) => {
//     fs.writeFileSync(path.resolve(__dirname, 'battle.config.json'), JSON.stringify(config, null, 2), 'utf-8');
//   });
